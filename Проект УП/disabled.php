...
$disabilityType = isset($_GET['disabilityType']) ? $_GET['disabilityType'] : '';
$notes = isset($_GET['notes']) ? $_GET['notes'] : '';

// Создание SQL-запроса с учетом фильтров
$query = "SELECT * FROM DisabledStudents WHERE 1=1";
...
if (!empty($disabilityType)) {
    $query .= " AND DisabilityType LIKE '%$disabilityType%'";
}
...
<body>
    ...
    <main>
        <section>
            <h2>Список инвалидов</h2>
            <form method="get" action="disabled.php">
                ...
                <input type="text" id="disabilityType" name="disabilityType" placeholder="Введите вид инвалидности" value="<?php echo htmlspecialchars($disabilityType); ?>">
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            echo "<tr>
                                ...
                                <td>{$row['StatusEnd']}</td>
                                <td>{$row['DisabilityType']}</td>
                                <td>{$row['Notes']}</td>
                            </tr>";
                        }
                    } else {
                        echo "<tr><td colspan='6'>Нет данных об инвалидах</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
